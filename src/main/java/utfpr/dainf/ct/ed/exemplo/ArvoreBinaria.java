...
/**
 * UTFPR - Universidade Tecnológica Federal do Paraná DAINF - Departamento Acadêmico de Informática
 *
...
public class ArvoreBinaria<E>
{
	 ...
	 */
	public ArvoreBinaria(E valor) { this.valor = valor; }

	/**
	 * Insere uma subárvore à esquerda deste nó. A subárvore à esquerda deste nó é inserida na folha mais à esquerda da subárvore inserida.
	 *
	...
	/**
	 * Insere uma subárvore à direita deste nó. A subárvore à direita deste nó é inserida na folha mais à direita da subárvore inserida.
	 *
	 * @param a A subárvore a ser inserida.
	 * @return A subárvore inserida.
	 */
	...
	/**
	 * Implementação padrão que exibe o valor armazenado no nó usando o método {@code toString()}. Pode ser sobrecarregado em classes derivadas para implementar outras formas de visita.
	 *
	 * @param no O nó a ser visitado
	 */
	protected void visita(ArvoreBinaria<E> no) { System.out.print(" " + no.valor); }
	 ...
	 */
	public void visitaEmOrdem() { visitaEmOrdem(this); }
	...
	/**
	 * Visita os nós da subárvore em pos-ordem.
	 *
	 * @param raiz A raiz da subárvore
	 */
	...
	/**
	 * Visita os nós da árvore em pos-ordem a partir da raiz.
	 */
	...
	private void inicializaPilha() { if (pilha == null) pilha = new Stack<>(); }

	private void inicializaFila() { if (fila == null) fila = new ArrayDeque<>(); }

	/**
	 * Reinicia o percurso a partir do início. Deve ser chamado após percorrer toda a árvore para realizar novo percurso ou para voltar ao início a qualquer momento.
	 */
	...
	public ArvoreBinaria<E> proximoPreOrdem()
	{
		...
		ultimoVisitado = pilha.empty() ? null : pilha.pop();
	...
	public ArvoreBinaria<E> proximoPosOrdem()
	{
		...
		ArvoreBinaria<E> atual;
		if (!pilha.empty())
		{
			atual = pilha.peek();
			if (atual.direita != null)
			{
				if (atual.direita != ultimoVisitado)
				{
					ultimoVisitado = atual.direita;
					pilha.push(ultimoVisitado);
					percorreEsquerda();
				}
			}
	...
	private boolean percorreEsquerda()
	{
		boolean resultado = false;
		ultimoEsquerda();
		while (proximoDireita())
		{
			resultado = true;
			ultimoEsquerda();
		}
	...
	private boolean ultimoEsquerda()
	{
		boolean resultado = proximoEsquerda();
		if (!resultado)
			return false;
		while(resultado)
			resultado = proximoEsquerda();
		return true;
	}
	
	private boolean proximoEsquerda()
	{
		boolean resultado = ultimoVisitado != null;
		if (resultado)
		{
			resultado = ultimoVisitado.esquerda != null;
			if (resultado)
			{
				ultimoVisitado = ultimoVisitado.esquerda;
				pilha.push(ultimoVisitado);
	...
	private boolean proximoDireita()
	{
		boolean resultado = ultimoVisitado != null;
		if (resultado)
		{
			resultado = ultimoVisitado.direita != null;
			if (resultado)
			{
				ultimoVisitado = ultimoVisitado.direita;
				pilha.push(ultimoVisitado);
	...
	private boolean filaParaPilha()
	{
		boolean resultado = !fila.isEmpty();
		if (resultado)
		{
			ultimoVisitado = fila.poll();
			pilha.push(ultimoVisitado);
		}
		else
			ultimoVisitado = null;
	...
	private boolean pilhaParaFila()
	{
		...
		if (resultado)
		{
			ultimoVisitado = pilha.pop();
			if (ultimoVisitado.direita != null)
				fila.addFirst(ultimoVisitado.direita);
			if (ultimoVisitado.esquerda != null)
				fila.addFirst(ultimoVisitado.esquerda);
		}
		return resultado;
	}
	
	private boolean esvaziaPilha()
	{
		boolean resultado = pilhaParaFila();
		if (!resultado)
			return false;
		while(resultado)
			resultado = pilhaParaFila();
		return true;
	}
	...
	/**
	 * Retorna a árvore esquerda.
	 *
	 * @return A árvore esquerda.
	 */
	protected ArvoreBinaria<E> getEsquerda() { return esquerda; }
	 ...
	 */
	protected ArvoreBinaria<E> getDireita() { return direita; }
	 ...
	 */
	protected void setEsquerda(ArvoreBinaria<E> esquerda) { this.esquerda = esquerda; }
	 ...
	 */
	protected void setDireita(ArvoreBinaria<E> direita) { this.direita = direita; }
